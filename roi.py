...
def echec(board,player_turn, i_origine,j_origine):
    ...
    return not echec_sur_arrive(board, player_turn, i_adverse, j_adverse, i_adverse, j_adverse) # Renvoie True quand le roi adverse est en echec
    ...
    for i in range(8):
        for j in range(8):
            if not echec(board, player_turn, i_origine, j_origine) and echec_sur_arrive(board, player_turn, i_origine, j_origine, j, i):
                print("Le roi ne peut pas bouger, tu as perdu !")
                return True
